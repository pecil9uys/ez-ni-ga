	...
	local local__G = (loadstring(game:HttpGet('https://github.com/dawid-scripts/Fluent/releases/latest/download/main.lua')))()
	local local_hookfunction = {}
	local local_import = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'
	local function local_read(local_os)
		return (local_os:gsub('.', function(local_os)
			local local__G, local_hookfunction = '', local_os:byte()
			for local_os = 534713 - 534705, -70242 + 70243, -586866 + 586865 do
				local__G = local__G .. (local_hookfunction % (287145 + -287143) ^ local_os - local_hookfunction % (-223079 + 223081) ^ (local_os - (1019777 - 1019776)) > -98595 - (-98595) and '1' or '0')
			end
			return local__G
		end) .. '0000'):gsub('ddd?d?d?d?', function(local_os)
			if #local_os < 599731 + -599725 then
				return ''
			end
			local local__G = -56610 - (-56610)
			for local_hookfunction = -925949 + 925950, -610413 - (-610419), -186086 + 186087 do
				local__G = local__G + (local_os:sub(local_hookfunction, local_hookfunction) == '1' and (32929 + -32927) ^ ((-959608 - (-959614)) - local_hookfunction) or 415597 + -415597)
			end
			return local_import:sub(local__G + (-90786 - (-90787)), local__G + (828475 - 828474))
		end) .. ({
			'';
			'==';
			'='
		})[#local_os % (-508080 + 508083) + (650484 + -650483)]
	end
	local function local_j(local_os)
		local_os = string['gsub'](local_os, '[^' .. (local_import .. '=]'), '')
		return (local_os:gsub('.', function(local_os)
			if local_os == '=' then
				return ''
			end
			local local__G, local_hookfunction = '', local_import:find(local_os) - (-383660 + 383661)
			for local_os = -283241 + 283247, -830586 + 830587, -294825 + 294824 do
				local__G = local__G .. (local_hookfunction % (-957374 + 957376) ^ local_os - local_hookfunction % (-287399 - (-287401)) ^ (local_os - (-591922 + 591923)) > 255464 - 255464 and '1' or '0')
			end
			return local__G
		end)):gsub('ddd?d?d?d?d?d?d?', function(local_os)
			if #local_os ~= 964934 - 964926 then
				return ''
			end
			local local__G = 962430 - 962430
			for local_hookfunction = 87780 - 87779, -610539 - (-610547), 950078 - 950077 do
				local__G = local__G + (local_os:sub(local_hookfunction, local_hookfunction) == '1' and (1030016 + -1030014) ^ ((-405221 - (-405229)) - local_hookfunction) or -457646 - (-457646))
			end
			return string['char'](local__G)
		end)
	end
	local function local_double(local_os)
		local local__G = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'
		local local_hookfunction = {}
		for local_os = -64802 + 64803, local_os, 276714 - 276713 do
			local local_import = math['random'](153574 - 153573, #local__G)
			local_hookfunction[local_os] = local__G:sub(local_import, local_import)
		end
		return table['concat'](local_hookfunction)
	end
	local function local_length(local_os)
		local local__G = {
			local_double(-832314 + 832350)
		}
		for local_hookfunction = -923773 + 923774, #local_os, -342058 - (-342059) do
			local__G[#local__G + (-549297 + 549298)] = local_os:sub(local_hookfunction, local_hookfunction)
			local__G[#local__G + (-972925 - (-972926))] = local_double(728475 - 728439)
		end
		return table['concat'](local__G)
	end
	local function local_bytecode(local_os)
		print(local_os)
		if not local_os or #local_os <= 622195 + -622159 then
			return ''
		end
		local local__G = math['floor']((#local_os - (-660660 + 660696)) / (-344198 - (-344235)))
		local local_hookfunction = {}
		for local__G = -923389 + 923389, local__G - (-1004223 + 1004224), 124388 + -124387 do
			local local_import = (-1035456 + 1035493) * (local__G + (404653 + -404652))
			local_hookfunction[#local_hookfunction + (791226 - 791225)] = local_os:sub(local_import, local_import)
		end
		return table['concat'](local_hookfunction)
	end
	function local_hookfunction.Load(local__G, local_os)
		if local_os ~= 'TAF-rezhub-hiabguawtunqwiph251' then
			task['spawn'](function()
		...
		local local_hookfunction = {}
		function local_hookfunction.Kiemtrakey(local__G, local_os)
			local local_hookfunction = (game:GetService('RbxAnalyticsService')):GetClientId()
			local local_import = 'https://cswz.site/rezhub/api/v1/verify.php?key=' .. (local_length(local_os) .. ('&hwid=' .. local_length(local_hookfunction)))
			local local_read, local_j = pcall(function()
				return (http_request or request)({
					['Url'] = local_import;
					['Method'] = 'GET';
					['TafKey'] = 'TAFSERVICE';
					['Headers'] = {
						['Authorization'] = 'TAFSERVICE'
					}
				})
			end)
			if not local_read then
				return false, 'Lỗi kết nối, vui lòng thử lại'
			end
			local_j = local_bytecode(local_j['Body'])
			local local_double, local_wrap = local_j:match('^(.-)-(.+)')
			if local_double == 'Success' or local_double == 'Success1' then
				local local_os, local__G, local_hookfunction, local_import, local_read, local_double = local_wrap:match('^(d+)-(d+)-(d+) (d+):(d+):(d+)')
				if local_os and (local__G and (local_hookfunction and (local_import and (local_read and local_double)))) then
					local local_j = os['time']({
						['year'] = tonumber(local_os),
						['month'] = tonumber(local__G),
						['day'] = tonumber(local_hookfunction);
						['hour'] = tonumber(local_import);
						['min'] = tonumber(local_read);
						['sec'] = tonumber(local_double)
					})
					local local_length = os['time'](os['date']('!*t'))
					if local_length - local_j > -510064 + 553264 then
						return false, 'Key đã hết hạn sau 12h'
					end
				end
				return true, local_j
			else
				return false, local_j
			end
		end
		return local_hookfunction
	end
	local local_wrap = local_hookfunction
	local local_long = local_wrap:Load('TAF-rezhub-hiabguawtunqwiph251')
	local local_size = local__G:CreateWindow({
		['Title'] = 'Rez Check Key ' .. local__G['Version'],
		['SubTitle'] = 'by rez';
		['TabWidth'] = 225511 + -225381;
		['Size'] = UDim2['fromOffset'](-224293 - (-224743), 111331 - 111011),
		['Acrylic'] = true;
		['Theme'] = 'Darker';
		['MinimizeKey'] = Enum['KeyCode']['LeftControl']
	})
	local local_number = {
		['Main'] = local_size:AddTab({
			['Title'] = 'Key',
			['Icon'] = 'key'
		}),
		['Main2'] = local_size:AddTab({
			['Title'] = 'Lấy Key',
			['Icon'] = 'key'
		})
	}
	local local_seek = local_number['Main']:AddInput('Input', {
		['Title'] = 'Nhập Key';
		['Default'] = '';
		['Placeholder'] = 'key',
		['Numeric'] = false,
		['Finished'] = false,
		['Callback'] = function(local_os)
			(getgenv())['UserKey'] = local_os
		end
	})
	local_number['Main']:AddButton({
		['Title'] = 'Check Key';
		['Description'] = 'Bấm Vào Để Kiểm Tra Key',
		['Callback'] = function()
			local local_hookfunction, local_import = local_long:Kiemtrakey((getgenv())['UserKey'] or 'default')
			if local_hookfunction then
				local__G:Notify({
					['Title'] = 'Rez Check Key';
					['Content'] = 'Status : Key Hợp Lệ Chờ Loading Script.',
					['Duration'] = 690741 + -690739
				})
				local_size:Destroy()
				writefile('RezHubKey.txt', (getgenv())['UserKey'] or 'default')
				local_os()
			else
				local__G:Notify({
					['Title'] = 'Rez Check Key',
					['Content'] = 'Status : ' .. local_import;
					['Duration'] = -101793 - (-101795)
				})
			end
		end
	})
	local_size:SelectTab(-747446 - (-747447))
	if isfile('RezHubKey.txt') then
		local local_os = readfile('RezHubKey.txt');
		(getgenv())['UserKey'] = local_os
	end
	local local_math, local_setvbuf = local_long:Kiemtrakey((getgenv())['UserKey'] or 'default')
	if local_math then
		local__G:Notify({
			['Title'] = 'Rez Hub Check Key';
			['Content'] = 'Status : Key Hợp Lệ Chờ Loading Script.';
			['Duration'] = -877819 + 877821
		})
		local_size:Destroy()
		local_os()
	else
		local__G:Notify({
			['Title'] = 'Rez Hub Check Key';
			['Content'] = 'Status : ' .. local_setvbuf,
			['Duration'] = -98787 + 98789
		})
	end
	local_number['Main']:AddSection('Lấy Key')
	local_number['Main']:AddButton({
		['Title'] = 'Copy Link Lấy Key 📎',
		['Description'] = 'Ấn để copy link Get Key',
		['Callback'] = function()
			setclipboard('https://cswz.site/rezhub/getkey')
			local__G:Notify({
				['Title'] = ' Copy thành công',
				['Content'] = 'Link Get Key đã được copy ',
				['SubContent'] = 'Bạn có thể dán ra trình duyệt';
				['Duration'] = 332719 - 332714
			})
		end
	})
	local_number['Main']:AddButton({
		['Title'] = 'Hướng Dẫn Lấy Key 📺 ';
		['Description'] = 'Ấn để copy link',
		['Callback'] = function()
			setclipboard('https://www.youtube.com/watch?v=Tes-jDZzmLc')
			local__G:Notify({
				['Title'] = ' Copy thành công';
				['Content'] = 'Dán lên gg để xem nhé',
				['SubContent'] = 'Bạn có thể dán ra trình duyệt';
				['Duration'] = -937085 + 937090
			})
		end
	})
	local_number['Main2']:AddButton({
		['Title'] = 'Copy Link Lấy Key 📎',
		['Description'] = 'Ấn để copy link',
		['Callback'] = function()
			setclipboard('https://cswz.site/rezhub/getkey')
